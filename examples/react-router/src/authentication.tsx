...
export const AuthContextProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const login = React.useCallback((user: User) => setUserState({ isLoading: false, user }), []);

  const logout = React.useCallback(() => setUserState({ isLoading: false, user: undefined }), []);

  return <AuthContext.Provider value={{ userState, login, logout, setUserState }}>{children}</AuthContext.Provider>;
};
